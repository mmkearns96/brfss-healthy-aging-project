...
First, limit the data to our age groups of interest (eg, 65 years and older). 
```{r}
...
```
Next, let's reduce our data-set down from 330 variables to 47 variables. See data dictionary for full explanation. 
```{r}
...
And finally, let's remove all respondents with "refused" or "don't know" in their responses. 
```{r}
#SEX. 
...
table(data$X_RFHLTH, useNA="ifany")
```
Let's take a look at our proportions of NA values within the dataset. 

```{r}
#Number of respondents with 50% NA values (31 missing columns).
sum(rowSums(is.na(data))>=24) # 9

#Number of respondents with 40% or more NA values (25 columns).
sum(rowSums(is.na(data))>=19) # 217

#Number of respondents with 30% or more NA values (19 columns). 
sum(rowSums(is.na(data))>=14) # 4366

#Number of rows with 4 or more columns with missing data. 
sum(rowSums(is.na(data))>3) # 43382

data_clean<-data[rowSums(is.na(data))<3,] #data with less than 10% #missing values

```
Let's check the NA proportions for our columns as well. 
```{r}
lapply(data_clean, function(var) {sum(is.na(var))}) 
#income will need to be recoded. 
```
...
data_clean$X_BMI5[data_clean$X_BMI5<18.02]<-18.02
data_clean$X_BMI5[data_clean$X_BMI5>44.08]<-44.08
...
data_clean$AVGDRNK[data_clean$AVGDRNK>21]<-21
...
data_clean$INCOME2[is.na(data_clean$INCOME2)]<-0
...
barplot(extract11,
        las=2,
        names.arg=c("Walking", "Gardening", "Other", "Bicycling\nMachine", "Aerobics\nClass", "Yard Work", "Bicycling", "Golf", "Weight lifting", "Calisthenics"),
        col="darksalmon",
        cex.names=0.8)
```
